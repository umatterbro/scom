...
class packet:
    ...
    class psds_s(section):
        ...
        class psds_sec(_psds):
            ...
            def __init__(self, headers: heads, data: str):
                if(not isinstance(headers, self.heads)): raise ValueError(f'headers must be of type heads, not {type(headers).__name__}')
    ...
    def create(src: ident, dst: ident, flags: int, req_num: int, tsmp: float = round(datetime.datetime.now().timestamp())):
        ...
        psdsec = packet.psds_s(tdsec.flags, 
                               ...
                               packet.psds_s.psds_sec(packet.psds_s.psds_sec.heads({'hello': 'world'}), 'erm what the sigma'),
                               packet.psds_s.psds_urg(),
                               packet.psds_s.psds_nsc(),
                               packet.psds_s.psds_rtp(),
